...
void CipherDecodePuzzle::Update(float, const InputManager& input) {
    if (solved) {
        return;
    }
...
void CipherDecodePuzzle::Render(TextRenderer& textRenderer, int screenWidth, int screenHeight, float alpha) const {
    ...
    textRenderer.RenderText("DECODE THE TOWN LEDGER", 74.0f, static_cast<float>(screenHeight) - 168.0f, 0.38f, textColor * alpha, screenWidth, screenHeight);

    textRenderer.RenderText(encoded, 74.0f, static_cast<float>(screenHeight) * 0.58f, 0.44f, accentColor * alpha, screenWidth, screenHeight);

    for (int i = 0; i < 4; ++i) {
        std::ostringstream line;
        line << (i + 1) << ". " << options[i];
        glm::vec3 color = (i == selectedIndex) ? glm::vec3(1.0f, 0.35f, 0.35f) : textColor;
        textRenderer.RenderText(line.str(), 74.0f, static_cast<float>(screenHeight) * 0.49f - (static_cast<float>(i) * 24.0f), 0.40f, color * alpha, screenWidth, screenHeight);
    }

    if (solved) {
        textRenderer.RenderText(solvedMessage, 74.0f, static_cast<float>(screenHeight) * 0.31f, 0.45f, glm::vec3(1.0f, 0.35f, 0.35f) * alpha, screenWidth, screenHeight);
    }
}
