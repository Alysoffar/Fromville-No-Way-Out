...
void VictorMemoryTimelinePuzzle::GuessMissing(int index) {
    if (index < 0 || index >= static_cast<int>(people.size())) return;
...
void VictorMemoryTimelinePuzzle::Update(float dt, const InputContext& input) {
    ...
    if (input.IsActionPressed(InputAction::PuzzleOption1)) GuessMissing(0);
    if (input.IsActionPressed(InputAction::PuzzleOption2)) GuessMissing(1);
    if (input.IsActionPressed(InputAction::PuzzleOption3)) GuessMissing(2);
    if (input.IsActionPressed(InputAction::PuzzleOption4)) GuessMissing(3);
...
void VictorMemoryTimelinePuzzle::Render(TextRenderer& textRenderer, int screenWidth, int screenHeight, float alpha) const {
    ...
    for (int i = 0; i < 4; ++i) {
        const glm::vec3 color = calmColor;
    ...
    textRenderer.RenderText("[1-4] select person  [ESC] steady", 72.0f, static_cast<float>(screenHeight) - 42.0f, 0.30f, titleColor * alpha, screenWidth, screenHeight);
}
