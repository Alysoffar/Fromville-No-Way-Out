...
void MemoryReplaySystem::Render() const {
    // Rendered by UI overlay system when Memory Mode is active
}
...
void MemoryReplaySystem::ActivateMemoryMode(float duration) {
    ...
    // Make witnessed events temporarily visible
...
void MemoryReplaySystem::UpdateMemoryMode(float dt) {
    // Called each frame to update VHS distortion and ghost replicas
    if (memoryModeActive) {
...
bool MemoryReplaySystem::CheckTimelineOrder(const std::vector<std::string>& playerOrder) {
    // Verify if player's ordering matches the correct chronological order
...
bool MemoryReplaySystem::CheckForContradiction(const std::string& eventA, const std::string& eventB) const {
    ...
    if (itA == eventMap.end() || itB == eventMap.end()) {
        return false;
    }

    // Contradictions: if person was at location A, they can't be at location B (temporal conflicts)
    if (itA->second.location == itB->second.location && itA->second.year == itB->second.year) {
        return true;
    }
